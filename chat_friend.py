...
class AloneChat(object):
    ...
    def logout(self):
        try:
            ...
            logging.debug("logout Failed", error)
    ...
    def addfirend(self):
        ...
        try:
            ...
            logging.info("添加的好友是：", self.friend)
    ...
    def refusefriend(self):
        ...
        try:
            ...
            logging.info("拒绝来自%s的添加好友申请" % self.friend)
    ...
    def agreefriend(self):
        ...
        try:
            ...
            logging.info("同意来自%s的添加好友申请" % self.friend)
    ...
    def sendMSfirend(self):
        ...
        try:
            ...
            logging.info("用户%s发送文本消息，内容为%s" %(self.user,message))
        ...
        try:
            if self.driver.find_element_by_xpath("//p[@class='current']"):
                ...
                if int(data) != message_num:
                    ...
                    logging.debug("Failed, Receive message is not complete, expect num is: %s ,now num is: %s" % (str(message_num),str(data)))
                    print u"Failed, Receive message is not complete, expect num is: %s ,now num is: %s" % (str(message_num),str(data))
                    return False
                else:
                    logging.info("Pass, New message is exist，num is: %s" % str(data))
                    print u"Pass, New message is exist，num is: %s" % str(data)
                    return True
        except Exception,e:
            self.screenshot("receive_%s.png" % time.strftime('%H_%M_%S'))
            logging.debug("No message received，verify failed",e)
    ...
    def addtoblack(self):
        ...
        try:
            ...
            if str(blackuser) == self.friend:
                logging.info("将好友%s成功加入到黑名单" % self.friend)
                print u"将好友%s成功加入到黑名单" % self.friend
                return True
            else:
                self.screenshot("%s_%s.png" % (funname, time.strftime('%H_%M_%S')))
                logging.debug("好友%s加入到黑名单失败" % self.friend)
    ...
    def removeblack(self):
        ...
        try:
            ...
            logging.info("将好友%s从黑名单中移除" % self.friend)
    ...
    def delfriend(self):
        ...
        try:
            ...
            logging.info("将好友%s删除" % self.friend)
    ...
    def cleanchat(self):
        ...
        try:
            ...
            logging.info("清空聊天历史记录成功")
            print u"清空聊天历史记录"
            return True
        except Exception, error:
            self.screenshot("%s_%s.png" % (funname, time.strftime('%H_%M_%S')))
            logging.debug("%s Failed, clean button is not found" % funname, error)
    ...
    def sendimage(self):
        ...
        try:
            ...
            logging.info("用户%s发送图片消息成功" % self.user)
        ...
        try:
            ...
            if self.driver.find_element_by_xpath("//div[@class='x-message-img']/img"):
                ...
                if code.status_code == 200:
                    logging.info("image messages receive success")
                    print "image messages receive success"
                    return True
                else:
                    self.screenshot("%s_%s.png" % (funname, time.strftime('%H_%M_%S')))
                    logging.debug("image messages receive failed")
                    print "image messages receive failed"
                    return False
        except Exception, error:
            self.screenshot("%s_%s.png" % (funname, time.strftime('%H_%M_%S')))
            logging.debug("not receive image messsage", error)
    ...
    def sendfile(self):
        ...
        try:
            ...
            logging.info("用户%s发送文件消息成功" % self.user)
    ...
    def receivefile(self):
        ...
        try:
            ...
            if self.driver.find_element_by_xpath("//div[@class='x-message-file']/div/div[2]/a"):
                ...
                if code.status_code == 200:
                    logging.info("File messages receive success")
                    print "File messages receive success"
                    return True
                else:
                    self.screenshot("%s_%s.png" % (funname, time.strftime('%H_%M_%S')))
                    logging.debug("File messages receive failed")
                    print "File messages receive failed"
                    return False
        except NoSuchElementException, error:
            self.screenshot("%s_%s.png" % (funname, time.strftime('%H_%M_%S')))
            logging.debug("not receive file messsage", error)
    ...
    def inviteAuVid(self,num):
        ...
        try:
            ...
            logging.info("%s invite %s audio or Video chat" %(self.user,self.friend))
            print "%s invite %s audio or Video chat" %(self.user,self.friend)
        except Exception, error:
            self.screenshot("%s_%s.png" % (funname, time.strftime('%H_%M_%S')))
            logging.debug("invite friend %s Video failed" % self.friend, error)
    ...
    def agreeVideo(self):
        ...
        try:
            ...
            if username == self.friend:
                ...
                logging.info("Agree friend %s Video success" % self.friend)
                print "Agree friend %s Video success" % self.friend
                return True
            else:
                self.screenshot("%s_%s_Failed.png" % (funname, time.strftime('%H_%M_%S')))
                logging.debug("Agree friend %s Video failed" % self.friend)
                print "Agree friend %s Video failed" % self.friend
                return False
        except Exception, error:
            self.screenshot("%s_%s_Failed.png" % (funname, time.strftime('%H_%M_%S')))
            logging.debug("Agree friend %s Video failed" % self.friend, error)
    ...
    def agreeAudio(self):
        ...
        try:
            ...
            if username == self.friend:
                self.screenshot("%s_%s_Pass.png" % (funname, time.strftime('%H_%M_%S')))
                logging.info("Agree friend %s Audio success" % self.friend)
                print "Agree friend %s Audio success" % self.friend
                return True
            else:
                self.screenshot("%s_%s_Failed.png" % (funname, time.strftime('%H_%M_%S')))
                logging.debug("Agree friend %s Audio failed" % self.friend)
                print "Agree friend %s Audio failed" % self.friend
                return False
        except Exception, error:
            self.screenshot("%s_%s_Failed.png" % (funname, time.strftime('%H_%M_%S')))
            logging.debug("Agree friend %s Audio failed" % self.friend, error)
        ...
        try:
            ...
            try:
                if self.driver.find_element_by_xpath("//video[@class='corner']"):
                    logging.info("refuse friend %s Audio or Video failed" % self.friend)
                    print "refuse friend %s Audio or Video failed" % self.friend
                    return False
            except NoSuchElementException:
                logging.debug("refuse friend %s Video or Audio success" % self.friend)
                print "refuse friend %s Video or Audio success" % self.friend
                return True
        except Exception, error:
            self.screenshot("%s_%s_Failed.png" % (funname, time.strftime('%H_%M_%S')))
            logging.debug("invite friend %s Video or Audio failed" % self.friend, error)
